...
const authenticateToken = async (req, res, next) => {
    ...
    const refreshToken = req.body.refreshToken; // Adjust based on your method of storing refresh token
    ...
    try {
        ...
        if (token) {
            ...
            try {
                ...
                console.log("Access granted", decodedAccess);
                req.user = decodedAccess; // Attach user info to the request object
                return next(); // Proceed to the next middleware or route handler
            } catch (err) {
                console.error("Access token not verified, checking for Refresh Token... ", err);
            }
        }
        ...
        if (refreshToken) {
            ...
            const decodedRefresh = jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET); // Use your refresh token secret
            console.log("Refresh granted", decodedRefresh);
            const user = await User.findById(decodedRefresh.id);

            if (!user || user.refreshToken !== refreshToken) {
                return res.status(403).json({ message: "Forbidden: Invalid refresh token" }); // Forbidden if refresh token is invalid
            }
            ...
            return res.status(403).json({ message: "Forbidden: No refresh token provided" }); // Handle case where no refresh token is provided
        }

    } catch (error) {
        console.error("Error during token verification:", error);
        return res.status(403).json({ message: "Forbidden: Token verification error" }); // Forbidden on any verification error
    }
};
